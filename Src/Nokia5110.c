...
void Nokia5110_OutChar(unsigned char data){
  int i;
  lcdwrite(DATA, 0x00);                 // blank vertical line padding (1st vertical padding)
  for(i=0; i<5; i=i+1){ // 5 colums is needed to represent the ascii character
		if(data > 0x20) { // error handling of out of range numbers
			lcdwrite(DATA, ASCII[data - 0x20][i]);
		} else {
			// do nothing since data is not within range
		}
  }
  lcdwrite(DATA, 0x00);                 // blank vertical line padding (2nd vertical padding)
}
...
// Inputs: n  16-bit unsigned number (short = 2 bytes)
// Outputs: none
// assumes: LCD is in default horizontal addressing mode (V = 0)
void Nokia5110_OutUDec(unsigned short n){
  if(n < 10){ // one decimal digit
    Nokia5110_OutString((unsigned char *)"    "); // right alignment (4 spaces)
    Nokia5110_OutChar(n+'0'); /* n is between 0 and 9 */
  } else if(n<100){
    Nokia5110_OutString((unsigned char *)"   "); // 3 spaces
    Nokia5110_OutChar(n/10+'0'); /* tens digit (integer division)*/
    Nokia5110_OutChar(n%10+'0'); /* ones digit (modulation to remove the tenth's digit)*/
  } else if(n<1000){
    Nokia5110_OutString((unsigned char *)"  "); // 2 spaces
    Nokia5110_OutChar(n/100+'0'); /* hundreds digit (integer division)*/
    n = n%100;
    Nokia5110_OutChar(n/10+'0'); /* tens digit */
    Nokia5110_OutChar(n%10+'0'); /* ones digit */
  }
  else if(n<10000){
    Nokia5110_OutChar(' '); // 1 space
  ...
  else { // no spaces since the number is 5 digits
...
void Nokia5110_DrawFullImage(const char *ptr){
  ...
  Nokia5110_SetCursor(0, 0); // top left corner
	// goes through all the pixels (8 since the height of the ascii code is 8 pixels)
  for(i=0; i<(MAX_X*MAX_Y/8); i=i+1){
    lcdwrite(DATA, ptr[i]);
  }
}
